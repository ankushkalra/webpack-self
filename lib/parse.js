  ...
  declarators.forEach(declarator => {
    ...
    switch (declarator.type) {
      case "VariableDeclarator":
        if (declarator.id.type === "Identifier" && context.options.overwrites.hasOwnProperty(declarator.id.name)) {
  ...
  switch (expression.type) {
    ...
    case expressionTypes.CallExpression:
      ...
      if (context.overwrite.indexOf("require") === -1 &&
        ...
        { ensure: 1, async: 1 }.hasOwnProperty(expression.callee.property.name)
      ) {
...
function parse(source, options) {
  try {
    ...
    console.log("really pasing failed: sourcej = ", source);
    console.log("really pasing failed: e = ", e);
    throw e;
  }
