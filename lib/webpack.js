...
module.exports = function webpack(context, moduleName, options, callback) {
  ...
  // console.log("options.parse.overwrites.process = ", options.parse.overwrites.process);
  ...
  buildDeps(context, moduleName, options, (err, depTree) => {
    // console.log("buildDeps callback");
    if (err) {
      console.log(err);
      throw err;
    }
    // console.log("buildDeps = ", buildDeps);

    var buffer = [];

    if (options.output) {
      console.log("options.output = ", options.output);
      console.log("options.outputPostfix = ", options.outputPostfix);
      ...
      for (var chunkId in depTree.chunks) {
        ...
        for (var moduleId in chunk.modules) {
          // console.log("moduleId = ", moduleId, depTree.modules);
          var modu = depTree.modulesById[moduleId];
